        ...
        private static byte[] GetMultipartFormData(Dictionary<string, object> postParameters, string boundary)
        {
            ...
            foreach (var param in postParameters)
            {
                ...
                if (param.Value is FileParameter)
                {
                    ...
                    formDataStream.Write(m_encoding.GetBytes(header), 0, m_encoding.GetByteCount(header));
                ...
                else
                {
                    string postData = string.Format("--{0}\r\nContent-Disposition: form-data; name=\"{1}\"\r\n\r\n{2}",
                        ...
                        param.Key,
                        param.Value);
                    formDataStream.Write(m_encoding.GetBytes(postData), 0, m_encoding.GetByteCount(postData));
                }
            }

            // Add the end of the request.  Start with a newline
            string footer = "\r\n--" + boundary + "--\r\n";
            formDataStream.Write(m_encoding.GetBytes(footer), 0, m_encoding.GetByteCount(footer));
